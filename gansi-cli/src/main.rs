...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match command {
        "-r" | "--register" => {
            let com_wrapper = ComWrapper::new(gansi_path)?;
        ...
        "-a" | "--all" => {
            let _gansi_wrapper = GansiComWrapper::new(gansi_path, pipe_suffix_ffi)?;
            let rt = Runtime::new()?;
    ...
    loop {
        ...
        let _client = tokio::spawn(async move {
            loop {
                ...
                match pipe_server::message::<GansiMessage>(&mut connected_client, 1000).await {
                    Ok(input_message) => {
                    ...
                    Err(err) => match err {
                        ...
                        PipeError::IoError(err) => log::trace!("IoError: {err}"),
                    },
                }
            }
        });
...
fn print_help() {
    println!("Usage: gansi-cli [OPTION] [dll_path] [pipe_suffix]");
    println!("Options:");
    ...
    println!();
    println!("Defaults: dll_path={GANSI_COM_DLL}, pipe_suffix={GANSI_PIPE_SUFFIX}");
}
