...
use shared::GansiMessage;
use tokio::runtime::Runtime;
use windows::Win32::System::Antimalware::*;

use super::Gansi_Impl;
use crate::utils::GansiEvent;
...
impl Gansi_Impl {
    pub fn report_script(&self, stream: &IAmsiStream) {
        ...
        let request_num = self.request_number.fetch_add(1, Ordering::AcqRel) + 1;

        let amsi_event = GansiEvent::new(
            process_info,
            app_name,
            content_name,
            content,
        ...
        if let Some(runtime) = &*RUNTIME {
            runtime.block_on(async {
                ...
                let _ = self
                    ...
                    .write()
                    .await
                    .send(GansiMessage::new(&amsi_event.to_string()))
                    .await
