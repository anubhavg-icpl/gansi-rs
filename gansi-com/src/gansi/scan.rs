...
use shared::DynResult;

use super::{Gansi_Impl, PsVersion, ScriptType, keywords::*};
...
impl Gansi_Impl {
    pub(super) fn scan_script(
        ...
        self.heuristic_scan((&tokens).into())
    }
...
mod tests {
    use super::*;
    use crate::gansi::Gansi;

    #[test]
    fn blacklist_keywords_contains_known_entries() {
        ...
        for expected in [
            ...
            assert!(
                ...
                "missing blacklisted keyword: {expected}"
            );
        }
    }
    ...
    #[test]
    fn sha_scan() {
        let gansi = Gansi::new().into_static();
        assert_eq!(
            gansi.scan_script("prompt".to_string(), ScriptType::PsCommand),
            ScanStatus::Clean
        );
    }
}
